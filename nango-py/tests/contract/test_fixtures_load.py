"""Contract fixtures must parse and carry the required envelope keys.

This is a structural guard. Behavioral parity is verified by the differential
harness in tests/differential once the Python adapter exists.
"""
...
FIXTURE_DIR = Path(__file__).resolve().parent / "integrations" / "fixtures" / "get_by_key"
...
def test_fixtures_have_required_keys() -> None:
    cases = _load_each()
    assert cases, "no contract fixtures found"
